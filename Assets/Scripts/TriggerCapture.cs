...
public class TriggerCapture : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        string path = "Assets/ObjectThumbnails/";
        string filename = string.Format("capture_{0}.png", DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss-fff"));
        int width = Screen.width;
        int height = Screen.height;
        if (UseSimple) {
            CaptureScreenshot.SimpleCaptureTransparentScreenshot(cam, width, height, path, filename);
        }
        else {
            CaptureScreenshot.CaptureTransparentScreenshot(cam, width, height, path, filename);
        }
    }
}
...
{
    public static void CaptureTransparentScreenshot(Camera cam, int width, int height, string path, string fileName)
    {
        ...
        File.WriteAllBytes(path + fileName, pngShot);
    ...
    public static void SimpleCaptureTransparentScreenshot(Camera cam, int width, int height, string path, string screengrabfile_path)
    {
